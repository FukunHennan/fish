...
export function motionCenter(profile, mode) {
  ...
  const balancedTurnOffset = Math.min(center - min, max - center) / 2;

  if (mode === "left") {
    return center - balancedTurnOffset;
  }
  if (mode === "right") {
    return center + balancedTurnOffset;
  }
...
export function motionAmplitude(profile, center, mode) {
  ...
  const straightCenter = clamp(profile.straightCenter, min, max, 90);
  const availableSwing = Math.min(straightCenter - min, max - straightCenter);
  const safeSwing = mode === "forward" || mode === "left" || mode === "right"
    ? availableSwing / 2
    : availableSwing;
