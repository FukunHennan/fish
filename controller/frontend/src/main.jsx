...
function App() {
  ...
  const [otaFeedback, setOtaFeedback] = useState("上传 firmware.bin 后即可对所选设备升级");
  ...
  async function uploadFirmware() {
    if (!firmwareFile || uploading) { setOtaFeedback("请先选择 firmware.bin"); return; }
  ...
  async function otaSelection() {
    ...
    if (!selectedOnline.length || sending) { if (!selectedOnline.length) setOtaFeedback("请先选择至少一台在线机器鱼"); return; }
    if (!window.confirm(`确定使用 ${firmwareInfo.name || "当前固件"} 升级所选 ${selectedOnline.length} 台设备？升级期间设备会停止并重启。`)) return;
    setSending(true);
    setOtaFeedback(`正在向 ${selectedOnline.length} 台设备创建 OTA 任务…`);
    ...
    const failed = results.filter((result) => result.status === "rejected").length;
    setOtaFeedback(failed ? `OTA 已启动 ${results.length - failed}/${results.length} 台` : `OTA 任务已发送 · ${results.length} 台`);
    setSending(false);
  }
    ...
    <main className="app-shell">
      <header className="topbar">
        ...
        <div className="system-status">
          ...
          <span className="top-metric"><b>{selectedOnline.length}</b><small>已选目标</small></span>
        ...
        <aside className="device-sidebar panel-surface">
          ...
          <div className="device-list">
            {visibleDevices.map((device) => {
              ...
              return (
                <button key={device.deviceId} className={`device-row ${selected ? "selected" : ""}`} disabled={!device.online} onClick={() => toggleDevice(device.deviceId)}>
          ...
          <div className="sidebar-section">
            ...
            <div className="group-grid"><button onClick={() => selectGroup("A")}>A 组</button><button onClick={() => selectGroup("B")}>B 组</button><button onClick={selectOnline}>全部</button></div>
          </div>
        </aside>
        ...
        <aside className="control-sidebar panel-surface">
          ...
          {paramMode === "sync" ? (
            ...
            <section className="control-section">
              ...
              <div className="individual-params">{selectedDevices.map((device) => <div key={device.deviceId}><strong>{deviceLabel(device)}</strong><span>{device.frequency ?? "—"} Hz · {device.amplitude ?? "—"}° · {Number(device.bias ?? 0) > 0 ? "+" : ""}{device.bias ?? 0}°</span></div>)}{!selectedDevices.length && <small>选择设备后显示各自参数</small>}</div>
            </section>
          )}

          <section className="control-section">
            <span className="sidebar-label">动作控制</span>
            <div className="motion-grid">{ACTIONS.map(([mode, label]) => <button key={mode} disabled={!selectedOnline.length || sending} onClick={() => sendToSelection(mode)}>{label}</button>)}</div>
            <p className="feedback" aria-live="polite">{feedback}</p>
          </section>

          <section className="control-section ota-section">
            <div className="ota-title"><div><span className="eyebrow">REMOTE OTA</span><h3>固件升级</h3></div><span className={`firmware-ready ${firmwareInfo.available ? "ready" : ""}`}>{firmwareInfo.available ? "READY" : "NO FIRMWARE"}</span></div>
            <label className="firmware-picker"><input type="file" accept=".bin,application/octet-stream" onChange={(event) => setFirmwareFile(event.target.files?.[0] || null)} /><span>{firmwareFile ? firmwareFile.name : "选择 firmware.bin"}</span></label>
            <button className="upload-button" disabled={!firmwareFile || uploading} onClick={uploadFirmware}>{uploading ? "上传校验中…" : "上传并校验"}</button>
            ...
            <button className="ota-start" disabled={!firmwareInfo.available || !selectedOnline.length || sending || uploading} onClick={otaSelection}>升级所选设备 {selectedOnline.length ? `(${selectedOnline.length})` : ""}</button>
            <p className="feedback" aria-live="polite">{otaFeedback}</p>
          </section>
        </aside>
      </div>

      <footer className="app-footer"><span>单鱼 / 多鱼选择集控制 · OTA 固件由中央控制器校验后在局域网分发</span><span>{selectedOnline.length ? `当前目标：${selectedOnline.length} 台在线设备` : "当前未选择在线设备"}</span></footer>
    </main>
  );
}
