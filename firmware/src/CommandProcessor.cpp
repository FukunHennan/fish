...
String CommandProcessor::process(String c){
    ...
    else if(c.startsWith("FREQ:")){float f=c.substring(5).toFloat();auto s=motion_.snapshot();if(!motion_.setTuning(f,s.amplitude))return "参数越界";}
    else if(c.startsWith("AMP:")){float a=c.substring(4).toFloat();auto s=motion_.snapshot();if(!motion_.setTuning(s.frequency,a))return "参数越界";}
    else return "未知命令";
    return "OK";
}
