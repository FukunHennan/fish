...
| 文档 | 用途 |
|---|---|
| [系统架构与开发方案](项目架构与开发方案.md) | 了解进程边界、WebRTC 链路、控制权状态、持久化和未完成能力 |
| [OTA 与图形化扩展平台架构](OTA与图形化扩展平台架构.md) | 了解 OTA、设备能力、扩展包和图形化编程的演进架构 |
| [前端赛事端技术方案](前端赛事端技术方案.md) | 了解赛事端 GUI 的 React/TypeScript、动效、状态管理和内网穿透边界 |
| [部署与公网访问](部署与公网访问.md) | 启动电脑端服务、检查公网和 FRP |
| [桌面启动与部署](桌面启动与部署.md) | fishctl 一键启停、桌面菜单与 systemd |
| [项目统一参数](项目统一参数.md) | 查看端口、心跳、视频和控制参数 |
| [维护计划](维护计划.md) | 了解后续整理顺序和修改规则 |
| [指导性文件总览](指导性文件总览.md) | 按使用场景整理当前有效文档、历史设计和整理建议 |
...
本目录的架构、部署和统一参数说明当前实现及其验证边界。`superpowers/specs/` 和 `superpowers/plans/` 保存历史设计与开发计划，其中的超时数值、角色或待办不能直接当作当前实现；发生冲突时以当前模块代码和本目录说明为准。本项目当前仅面向内部研发和受控环境，暂不把公网攻击、商用部署或其他安全加固作为开发目标；维护工作集中于设备功能、控制可靠性、视觉效果、异常恢复和文档一致性。

- [项目维护规范](项目维护规范.md)：目录归属、入口约定和多用户架构目标与差距。

- [多用户控制与视频使用说明](多用户控制与视频使用说明.md)：账号归属、浏览器恢复、按键、分辨率、裁剪和故障排查。

- [会话与性能维护记录](2026-09-04会话与性能维护.md)：持久化、GPU 修复、实测数据和控制代码清理。
